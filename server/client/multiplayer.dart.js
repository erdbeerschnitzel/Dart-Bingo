...
$.GameHandler = function(gameevent) {
  if ($.first !== true) {
    ...
    $.add$1($.ws.get$on().get$message(), new $.Closure3());
...
$.ReadyHandler = function(readyevent) {
  if ($.gameStarted !== true) {
    if ($.eqB($.document().query$1('#startGame').get$value(), 'I\'m ready!')) {
      ...
      var t1 = $.query('#startGame');
      $.callTypeCast(t1, 'is$ButtonElement').set$value('I\'m not ready!');
    } else {
  ...
  for (var i = 0; i < 5; ++i) {
    for (var t1 = i === 2, x = 0; x < 5; ++x) {
      ...
      if (!(t1 && x === 2)) {
        ...
        $.add$1(t2.el_1.get$on().get$click(), new $.Closure7(t2));
      }
    }
  }
};
...
$.defineProperty(Object.prototype, 'is$ButtonElement', function() { return false; });

...
Isolate.$defineClass("HashMapImplementation", "Object", ["_numberOfDeleted", "_numberOfEntries", "_loadLimit", "_values", "_keys?"], {
 ...
 operator$indexSet$2: function(key, value) {
  ...
  if (!t1) {
    t1 = $.index(this._keys, index) === $.CTC7;
  }
  if (t1) {
...
Isolate.$defineClass("Gamecard", "Object", ["addedNumbers", "fields?"], {
 getRandomNumber$0: function() {
  ...
  while (true) {
    ...
    if (!t1) {
      t1 = $.geB($.indexOf$1(this.addedNumbers, a), 0);
    }
    if (!t1) break;
...
Isolate.$defineClass("_ChildrenElementList", "Object", ["_childElements", "_element"], {
 ...
 _toList$0: function() {
  ...
  for (var len = $.get$length(this._childElements), i = 0; $.ltB(i, len); ) {
...
$.getRandomNumber = function() {
  ...
  while (true) {
    ...
    if (!t1) {
      t1 = $.geB($.indexOf$1($.addNumbers, a), 0);
    }
    if (!t1) break;
    a = $.toInt($.mul($.random(), 100));
  }
...
$.createCard = function(card, forComputer) {
  var t1 = forComputer === true;
  for (var t2 = $.iterator(card.get$fields()), cardstring = '', x = 0, i = 0; t2.hasNext$0() === true; ) {
    var t3 = t2.next$0();
    cardstring = cardstring + '<tr>';
    var t4 = i < 5;
    for (t3 = $.iterator(t3); t3.hasNext$0() === true; ) {
      t3.next$0();
      var t5 = x < 5;
      if (t1) {
        if (t5) {
        ...
        if (t5) {
          cardstring = $.add($.add($.add(cardstring + '<td id="c', i), x) + '"' + 'class=top>', $.index($.index(card.get$fields(), i), x)) + '</td>';
        }
      } else {
        if (t5) {
          t5 = t4;
...
$.checkBingo = function(card) {
  for (var result = true, deb = '', i = 0; i < 5; ) {
    var t1 = i === 2;
    for (var x = 0; x < 5; ) {
      if (t1) {
        var t2 = x === 2;
      ...
      if (t2) {
        ...
        deb = $.add(deb + ('' + $.stringToString(i) + $.stringToString(x) + ': '), $.index($.index(card.get$fields(), i), x)) + ' ';
      }
      x = x + 1;
    }
    if (result) {
  ...
  for (var i = 0; i < 5; ) {
    ...
    for (var x = 0; x < 5; ) {
      ...
      if (t3) {
        ...
        t2.el_1 = $.document().query$1('#p' + $.stringToString(i) + $.stringToString(x));
